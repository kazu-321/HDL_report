...
## 解説
TopModuleモジュール内で、SW[3:0]とSW[7:4]をそれぞれm_seven_segmentモジュールに入力し、対応する7セグメント表示をHEX2とHEX3に割り当てている。
add4モジュールでSW[3:0]とSW[7:4]の和を計算し、その結果を再びm_seven_segmentモジュールに入力してHEX0に表示している。
また、繰り上がり(co)をm_seven_segmentモジュールに入力してHEX1に表示している